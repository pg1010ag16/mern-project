...
// })
route.post('/loginjoin',(req,res)=>{
    var userid = req.body.userid;
    var pwd = req.body.pwd;
    const User = require('../models/user');
    const userObject = new User(userid,pwd);
    const userOperations = require('../db/helpers/usercrud');
    const promise = userOperations.searchUserAndRole(userObject);
    promise.then(data=>{
        res.json(data);
    }).catch(err=>{
        res.json(err);
...
route.post('/login',(req,res,next)=>{
    ...
    promise.then(doc=>{
        if(doc){
            ...
            const token = tokenObj.generateToken(userid);
            res.render('dashboard',{uid:doc.userid,token:token,roleName:doc.roleName,
